...
// Função par alterar entre os temas
function toggleMode(){
...
function updateTextColor(){
    currentColor = documentElement.classList.contains('light') ?
    'black' : "#fff";
...
function animateText() {
    if (isTyping) {
        ...
        if (index > 1) {
            titleElement.textContent = text.slice(0, index - 1); //Remove uma letra título
            index--;
        } else {
...
// Inicia animação quando carrega a página
...
function startAutoSlide() {
    autoSlideInterval = setInterval(nextSlide, 5000); // Avança o slide a cada 5 segundos// Função para reiniciar a transição automática
}
...
window.addEventListener('load', () => {
    ...
    // Pausa a transição automática ao passar o mouse sobre o carrossel carouselSlides.parentElement.addEventListener('mouseenter', () => {
